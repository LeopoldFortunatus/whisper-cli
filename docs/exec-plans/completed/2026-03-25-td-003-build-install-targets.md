# TD-003 Build And Install Targets
...
3. Docs могут описать contract иначе, чем его реализует `Makefile`.
...
4. Прогнать `make build`, `make install` с переопределённым `INSTALL_DIR`, `make ci`, затем закрыть debt и перенести plan в `completed/`.
...
- Validation завершён: `make build`, `make install INSTALL_DIR=/tmp/whisper-cli-install`, `make ci`.
